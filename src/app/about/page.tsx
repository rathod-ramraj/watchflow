...
const TEAM = [
  {
    name: "rathod-ramraj",
    role: "Founder, Curator & Maintainer",
    avatar: "https://github.com/rathod-ramraj.png",
    bio: "Passionate about making entertainment accessible to everyone. Constantly hunting for the best streaming sites to add to the collection and keeping the project running smoothly.",
    github: "https://github.com/rathod-ramraj",
  },
];
    ...
    <main className="mx-auto max-w-4xl px-6 py-16 md:px-8">
      ...
      <section className="watchflow-card mb-12 p-8 md:p-10 rounded-[24px]">
        ...
        <p className="text-sm md:text-base leading-relaxed text-[var(--fg-muted)]">
          ...
          Whether you are a movie buff, anime fan, manga reader, or sports enthusiast — we have you covered.
        </p>
      </section>
        ...
        <div className="flex justify-center">
          {TEAM.map((m) => (
            <div key={m.name} className="watchflow-card flex max-w-md flex-col items-center p-10 text-center rounded-[24px] shadow-lg shadow-black/30 hover:-translate-y-1 hover:scale-[1.02] hover:watchflow-glow duration-300">
              ...
              <p className="mt-1.5 text-[9px] font-extrabold uppercase tracking-[0.2em] text-[var(--accent)]">{m.role}</p>
              <p className="mt-4 text-sm font-medium leading-relaxed text-[var(--fg-muted)]">{m.bio}</p>
              <a
                href={m.github}
                target="_blank"
                rel="noreferrer"
                className="group mt-5 inline-flex h-10 w-10 items-center justify-center rounded-full bg-[var(--bg-elev)] border border-[var(--border)] text-[var(--fg-muted)] hover:text-white hover:border-[var(--border-strong)] transition-all hover:scale-110 active:scale-95"
              >
                <GithubIcon size={16} />
              </a>
            </div>
